...
@pytest.mark.parametrize(
    "number, lst, expected_result",
    [
        (
            2,
            [1, 2, 3],
            [2, 4, 6],
        ),  # Test case 1: number=2, lst=[1, 2, 3], expected_result=[2, 4, 6]
        (
            3,
            [4, 5, 6],
            [12, 15, 18],
        ),  # Test case 2: number=3, lst=[4, 5, 6], expected_result=[12, 15, 18]
        (
            0,
            [7, 8, 9],
            [0, 0, 0],
        ),  # Test case 3: number=0, lst=[7, 8, 9], expected_result=[0, 0, 0]
        (
            -1,
            [-1, -2, -3],
            [1, 2, 3],
        ),  # Test case 4: number=-1, lst=[-1, -2, -3], expected_result=[1, 2, 3]
    ],
)
...
@allure.description(
    "Import the function calculation the mean value from a list of ints from SciPy library."
)
@pytest.mark.parametrize(
    "input_values, expected_output", [([1, 2, 3], 2), ([3, 3, 3], 3), ([2, 2, 2], 2)]
)
